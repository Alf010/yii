...
abstract class CDbSchema extends CComponent
{
	...
	public function getSchemaNames($refresh=false)
	{
        ...
        {
            $qcDuration=$this->disableQueryCache();

            if(!isset($this->_cacheExclude['schemaNames']) &&
            ...
            {
                $key='yii:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':schemaNames';
                $names=$cache->get($key);
                if($refresh===true || $names===false)
                {
                    $names = $this->findSchemaNames();
                    $cache->set($key,$names,$duration);
                }
            ...
            $this->enableQueryCache($qcDuration);
	...
	{
//        throw new CDbException(Yii::t('yii','{class} does not support fetching all schema names.',
//                                      array('{class}'=>get_class($this))));
    ...
    protected function disableQueryCache()
    {
        // temporarily disable query caching
        if($this->_connection->queryCachingDuration>0)
        {
            $duration=$this->_connection->queryCachingDuration;
    ...
    protected function enableQueryCache($duration)
    {
        if(isset($duration))  // re-enable query caching
            $this->_connection->queryCachingDuration=$duration;
    }
    ...
    protected function getCachedTableNames($include_views=true,$refresh=false)
    {
        ...
        if(!isset($this->_cacheExclude['tableNames']) &&
           ($duration=$this->_connection->schemaCachingDuration)>0 &&
           $this->_connection->schemaCacheID!==false &&
           ($cache=Yii::app()->getComponent($this->_connection->schemaCacheID))!==null)
        {
            $key='yii:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':tableNames';
            $names=$cache->get($key);
            if($refresh===true || $names===false)
            {
                $names = array();
    ...
    public function getProcedure($name,$refresh=false)
    {
        ...
        else
        {
            ...
            {
                $key='yii:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':'.$name;
                $procedure=$cache->get($key);
                if($refresh===true || $procedure===false)
                {
                    $procedure=$this->loadProcedure($realName);
                    if($procedure!==null)
                        $cache->set($key,$procedure,$duration);
                }
    ...
    {
        throw new CDbException(Yii::t('yii','{class} does not support loading stored procedures.',
                                      array('{class}'=>get_class($this))));
    }
    ...
    {
        throw new CDbException(Yii::t('yii','{class} does not support calling stored procedures.',
                                      array('{class}'=>get_class($this))));
    }
    ...
    protected function getCachedProcedureNames($refresh=false)
    {
        ...
        if(!isset($this->_cacheExclude['procedureNames']) &&
           ($duration=$this->_connection->schemaCachingDuration)>0 &&
           $this->_connection->schemaCacheID!==false &&
           ($cache=Yii::app()->getComponent($this->_connection->schemaCacheID))!==null)
        {
            $key='yii:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':procedureNames';
            $names=$cache->get($key);
            if($refresh===true || $names===false)
            {
                $names = array();
     ...
     * @throws CDbException if current schema does not support fetching all stored procedure names
     * @return array all stored procedure names in the database.
     */
    protected function findProcedureNames($schema='')
    {
        throw new CDbException(Yii::t('yii','{class} does not support fetching all stored procedure names.',
                                      array('{class}'=>get_class($this))));
    }
    ...
    public function getFunction( $name, $refresh = false )
    {
        ...
        else
        {
            ...
            if ( !isset( $this->_cacheExclude[$name] ) && ( $duration = $this->_connection->schemaCachingDuration ) > 0 &&
                 $this->_connection->schemaCacheID !== false &&
                 ( $cache = Yii::app()->getComponent( $this->_connection->schemaCacheID ) ) !==
                 null
            )
            {
                $key = 'yii:dbschema' . $this->_connection->connectionString . ':' . $this->_connection->username . ':' . $name;
                $function = $cache->get( $key );
                if ( $refresh === true || $function === false )
                {
                    $function = $this->loadFunction( $realName );
                    if ( $function !== null )
                    {
                        $cache->set( $key, $function, $duration );
                    }
                }
    ...
    protected function getCachedFunctionNames($refresh=false)
    {
        ...
        if(!isset($this->_cacheExclude['functionNames']) &&
           ($duration=$this->_connection->schemaCachingDuration)>0 &&
           $this->_connection->schemaCacheID!==false &&
           ($cache=Yii::app()->getComponent($this->_connection->schemaCacheID))!==null)
        {
            $key='yii:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':functionNames';
            $names=$cache->get($key);
            if($refresh===true || $names===false)
            {
                $names = array();
     ...
     * @throws CDbException if current schema does not support fetching all stored function names
     * @return array all stored function names in the database.
     */
    protected function findFunctionNames($schema='')
    {
        throw new CDbException(Yii::t('yii','{class} does not support fetching all stored function names.',
                                      array('{class}'=>get_class($this))));
    }
    ...
    {
        throw new CDbException(Yii::t('yii','{class} does not support loading stored functions.',
                                      array('{class}'=>get_class($this))));
    }
    ...
    {
        throw new CDbException(Yii::t('yii','{class} does not support calling stored functions.',
                                      array('{class}'=>get_class($this))));
    }
	...
	public function refresh()
	{
		if(($duration=$this->_connection->schemaCachingDuration)>0 && $this->_connection->schemaCacheID!==false && ($cache=Yii::app()->getComponent($this->_connection->schemaCacheID))!==null)
		{
            ...
            $key='yii:dbschema'.$this->_connection->connectionString.':'.$this->_connection->username.':tableNames';
            $cache->delete($key);
			foreach($this->_tableNames as $schema)
			{
